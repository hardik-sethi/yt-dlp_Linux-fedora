...
resolve_cookies_source() {
    ...
    case "$src" in
        brave)
            ...
            #
            # Cookie DB locations vary:
            #   - Older Chromium:  <profile>/Cookies
            #   - Modern Chromium: <profile>/Network/Cookies      (~mid-2024+)
            #   - Multi-profile:   "Default", "Profile 1", "Profile 2"...
            # Use `find` to locate the newest "Cookies" file at any depth
            # under each channel; the channel containing the newest one wins.
            local roots=(
                "$HOME/.config/BraveSoftware"
                "$HOME/.var/app/com.brave.Browser/config/BraveSoftware"
            )
            local best="" best_mtime=0 root channel cookies_file mtime
            for root in "${roots[@]}"; do
                [[ -d "$root" ]] || continue
                for channel in "$root"/*/; do
                    [[ -d "$channel" ]] || continue
                    cookies_file=$(find "$channel" -maxdepth 3 -type f -name Cookies \
                                   -printf '%T@\t%p\n' 2>/dev/null \
                                   | sort -nr | head -1 | cut -f2-)
                    [[ -n "$cookies_file" ]] || continue
                    mtime=$(stat -c %Y "$cookies_file" 2>/dev/null) || mtime=0
...
main() {
    ...
    # Bash doesn't expand a leading ~ inside a quoted variable; do it manually
    # so typing "~/Videos" at the prompt doesn't create a literal ~ folder.
    outdir="${outdir/#\~\//$HOME/}"
    [[ "$outdir" == "~" ]] && outdir="$HOME"
    mkdir -p "$outdir"
    ...
    if [[ -n "$COOKIES_FROM_BROWSER" ]]; then
        if [[ "$cookies_resolved" != "$COOKIES_FROM_BROWSER" ]]; then
            echo "  Cookies    : $COOKIES_FROM_BROWSER ${D}(→ ${cookies_resolved#*:})${R}"
        else
