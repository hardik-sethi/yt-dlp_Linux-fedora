...
main() {
    ...
    compress_mode=$(
        menu "Compression mode?" 2 \
            "Archive quality (large size)" \
            "Optimized HEVC (recommended)"
    )

    local cq="$DEFAULT_CQ"

    if [[ "$compress_mode" == "Optimized HEVC (recommended)" ]]; then
        cq=$(ask "HEVC CQ value (lower=better quality, larger file):" "$DEFAULT_CQ")
    fi
    ...
    if [[ "$compress_mode" == "Optimized HEVC (recommended)" ]]; then
        echo "  Compression: HEVC NVENC (CQ $cq)"
    else
    ...
    if [[ "$compress_mode" == "Optimized HEVC (recommended)" ]]; then
        ...
        local optimized_outfile="$outdir/${outname}.optimized.mkv"

        echo
        info "Optimizing video with HEVC NVENC..."
        echo "${D}This may take some time depending on video length.${R}"
        ...
        [[ -f "$optimized_outfile" ]] \
            || die "HEVC optimization failed"

        rm -f "$outfile"

        outfile="$optimized_outfile"
    fi
